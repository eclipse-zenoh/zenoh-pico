...
static inline z_result_t _z_serial_open_from_pins(const _z_serial_ops_t *ops, _z_sys_net_socket_t *sock, uint32_t txpin,
                                                  uint32_t rxpin, uint32_t baudrate) {
    return ops->open_from_pins(sock, txpin, rxpin, baudrate);
}

static inline z_result_t _z_serial_open_from_dev(const _z_serial_ops_t *ops, _z_sys_net_socket_t *sock, const char *dev,
                                                 uint32_t baudrate) {
    return ops->open_from_dev(sock, dev, baudrate);
}

static inline z_result_t _z_serial_listen_from_pins(const _z_serial_ops_t *ops, _z_sys_net_socket_t *sock,
                                                    uint32_t txpin, uint32_t rxpin, uint32_t baudrate) {
    return ops->listen_from_pins(sock, txpin, rxpin, baudrate);
}

static inline z_result_t _z_serial_listen_from_dev(const _z_serial_ops_t *ops, _z_sys_net_socket_t *sock,
                                                   const char *dev, uint32_t baudrate) {
    return ops->listen_from_dev(sock, dev, baudrate);
}

static inline void _z_serial_close(const _z_serial_ops_t *ops, _z_sys_net_socket_t *sock) { ops->close(sock); }

static inline size_t _z_serial_read(const _z_serial_ops_t *ops, _z_sys_net_socket_t sock, uint8_t *ptr, size_t len) {
    // flawfinder: ignore
    return ops->read(sock, ptr, len);
}

static inline size_t _z_serial_write(const _z_serial_ops_t *ops, _z_sys_net_socket_t sock, const uint8_t *ptr,
                                     size_t len) {
