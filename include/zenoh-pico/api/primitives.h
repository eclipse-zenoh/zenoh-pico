 ...
 * Parameters:
 *   cancellation_token: Pointer to token to cancel.
 *   timeout_ms: Timeout for cancel operation, if it is exceeded, the call will be interrupted and Z_ETIMEDOUT value
 * will be returned.
 *
 * Return:
 *   ``0`` in case of success (in this case all pending operations are guaranteed to be cancelled or terminated),
 ...
 * .. warning:: This API has been marked as unstable: it works as advertised, but it may be changed in a future release.
 */
z_result_t z_cancellation_token_cancel_with_timeout(z_loaned_cancellation_token_t *cancellation_token,
                                                    uint32_t timeout_ms);
