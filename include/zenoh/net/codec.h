...
#define ZN_DECLARE_ENCODE(name) \
    int zn_##name##_encode(z_wbuf_t *buf, uint8_t header, const zn_##name##_t *m)

#define ZN_DECLARE_ENCODE_NOH(name) \
    int zn_##name##_encode(z_wbuf_t *buf, const zn_##name##_t *m)

#define ZN_DECLARE_DECODE(name)                                             \
    zn_##name##_result_t zn_##name##_decode(z_rbuf_t *buf, uint8_t header); \
    void zn_##name##_decode_na(z_rbuf_t *buf, uint8_t header, zn_##name##_result_t *r)

#define ZN_DECLARE_DECODE_NOH(name)                         \
    zn_##name##_result_t zn_##name##_decode(z_rbuf_t *buf); \
    void zn_##name##_decode_na(z_rbuf_t *buf, zn_##name##_result_t *r)
