...
#define _ZN_FLAG_S_D 0x20 // 1 << 5 | LeasePeriod   if D==1 then the lease period is present
...
#define _ZN_FLAG_S_O 0x20 // 1 << 5 | Options       if O==1 then options are present
...
typedef union
{
    ...
    z_vec_t messages; // List of Zenoh Messages
} _zn_payload_t;
typedef struct
{
    int is_reliable;
