...
static _z_subscription_rc_t *register_local_subscription(const _z_keyexpr_t *expanded, uint16_t rid,
                                                         atomic_uint *counter, zp_locality_t allowed_origin) {
...
static void test_put_local_only_single(void) {
    ...
    _z_subscription_rc_t *subscription_rc =
        register_local_subscription(&expanded, rid, &g_local_put_delivery_count, ZP_LOCALITY_SESSION_LOCAL);
    ...
    bool delivered =
        _z_session_deliver_push_locally(&g_session, &keyexpr, &payload, &encoding, Z_SAMPLE_KIND_PUT, qos, &ts, NULL,
                                        Z_RELIABILITY_RELIABLE, &source_info, ZP_LOCALITY_SESSION_LOCAL);
    assert(delivered);
...
static void test_query_local_only_single(void) {
    ...
    _z_session_queryable_rc_t *queryable_rc =
        register_local_queryable(&expanded, &g_local_query_delivery_count, ZP_LOCALITY_SESSION_LOCAL);
    ...
    _z_n_qos_t qos = _z_n_qos_make(false, false, Z_PRIORITY_DEFAULT);
    z_result_t res = _z_query(&g_session, &keyexpr, NULL, 0, Z_QUERY_TARGET_DEFAULT, Z_CONSOLIDATION_MODE_LATEST, NULL,
                              NULL, query_reply_callback, query_dropper, NULL, 1000, NULL, qos,
                              Z_CONGESTION_CONTROL_BLOCK, ZP_LOCALITY_SESSION_LOCAL);
...
static void test_put_local_only_multiple(void) {
    ...
    _z_subscription_rc_t *sub_secondary =
        register_local_subscription(&expanded, rid, &local_put_secondary_count, ZP_LOCALITY_SESSION_LOCAL);
...
static void test_put_local_and_remote(void) {
    ...
    _z_subscription_rc_t *sub_primary =
        register_local_subscription(&expanded, rid, &g_local_put_delivery_count, ZP_LOCALITY_ANY);
    ...
    z_result_t res =
        _z_write(&g_session, &keyexpr, &payload, &encoding, Z_SAMPLE_KIND_PUT, Z_CONGESTION_CONTROL_BLOCK,
                 Z_PRIORITY_DEFAULT, false, &ts, NULL, Z_RELIABILITY_RELIABLE, &source_info, ZP_LOCALITY_SESSION_LOCAL);
    assert(res == _Z_RES_OK);
    assert(atomic_load_explicit(&g_local_put_delivery_count, memory_order_relaxed) == 1);
    assert(atomic_load_explicit(&g_network_send_count, memory_order_relaxed) == 0);
    ...
    res = _z_write(&g_session, &keyexpr, &payload, &encoding, Z_SAMPLE_KIND_PUT, Z_CONGESTION_CONTROL_BLOCK,
                   Z_PRIORITY_DEFAULT, false, &ts, NULL, Z_RELIABILITY_RELIABLE, &source_info, ZP_LOCALITY_ANY);
    assert(res == _Z_RES_OK);
    assert(atomic_load_explicit(&g_local_put_delivery_count, memory_order_relaxed) == 1);
...
static void test_query_local_only_multiple(void) {
    ...
    _z_session_queryable_rc_t *queryable_secondary =
        register_local_queryable(&expanded, &local_query_secondary_count, ZP_LOCALITY_SESSION_LOCAL);
...
static void test_query_local_and_remote(void) {
    ...
    _z_session_queryable_rc_t *queryable_primary =
        register_local_queryable(&expanded, &g_local_query_delivery_count, ZP_LOCALITY_ANY);
    ...
    assert(atomic_load_explicit(&g_query_reply_callback_count, memory_order_relaxed) == 2);
    assert(atomic_load_explicit(&g_query_drop_callback_count, memory_order_relaxed) == 1);
    assert(g_session._pending_queries == NULL);
...
static void test_put_remote_only_destination(void) {
    ...
    z_result_t res =
        _z_write(&g_session, &keyexpr, &payload, &encoding, Z_SAMPLE_KIND_PUT, Z_CONGESTION_CONTROL_BLOCK,
                 Z_PRIORITY_DEFAULT, false, &ts, NULL, Z_RELIABILITY_RELIABLE, &source_info, ZP_LOCALITY_REMOTE);
    assert(res == _Z_RES_OK);
...
static void test_subscriber_remote_only_origin(void) {
    ...
    _z_subscription_rc_t *sub =
        register_local_subscription(&expanded, rid, &g_local_put_delivery_count, ZP_LOCALITY_REMOTE);
...
static void test_query_remote_only_destination(void) {
    ...
    z_result_t res = _z_query(&g_session, &keyexpr, NULL, 0, Z_QUERY_TARGET_DEFAULT, Z_CONSOLIDATION_MODE_LATEST, NULL,
                              NULL, query_reply_callback, query_dropper, NULL, 1000, NULL, qos,
                              Z_CONGESTION_CONTROL_BLOCK, ZP_LOCALITY_REMOTE);
    assert(res == _Z_RES_OK);
...
static void test_queryable_remote_only_origin(void) {
    ...
    _z_session_queryable_rc_t *queryable_rc =
        register_local_queryable(&expanded, &g_local_query_delivery_count, ZP_LOCALITY_REMOTE);
    ...
    z_result_t res = _z_query(&g_session, &keyexpr, NULL, 0, Z_QUERY_TARGET_DEFAULT, Z_CONSOLIDATION_MODE_LATEST, NULL,
                              NULL, query_reply_callback, query_dropper, NULL, 1000, NULL, qos,
                              Z_CONGESTION_CONTROL_BLOCK, ZP_LOCALITY_ANY);
    assert(res == _Z_RES_OK);
    assert(atomic_load_explicit(&g_local_query_delivery_count, memory_order_relaxed) == 0);
    assert(atomic_load_explicit(&g_network_send_count, memory_order_relaxed) == 1);
