...
void test_cancel_get(void) {
    ...
    z_sleep_s(1);
    assert(z_cancellation_token_cancel_with_timeout(z_cancellation_token_loan_mut(&ct), 10000) == Z_OK);
    assert(z_cancellation_token_is_cancelled(z_cancellation_token_loan(&ct)));
    ...
    // Due to blocking query callback execution z_cancellation_token_cancel_with_timeout
    // will never terminate because of timeout, adn will always run till the end
    //
    /*printf("Check cancel with timeout fails if timeout is exceeded\n");
    t = 0;
    assert(z_cancellation_token_new(&ct) == Z_OK);
    assert(z_cancellation_token_clone(&ct_clone, z_cancellation_token_loan(&ct)) == Z_OK);
    z_get_options_default(&opts);
    ...
    z_query_drop(z_query_move(&q));
    z_sleep_s(1);
    assert(z_cancellation_token_cancel_with_timeout(z_cancellation_token_loan_mut(&ct), 1000) == Z_ETIMEOUT);
    z_sleep_s(5);
    assert(t == 3);*/
