...
static void expect_next_timeout(const z_loaned_fifo_handler_sample_t *handler, const char *expected,
                                size_t timeout_ms) {
    const size_t SLEEP_MS = 50;
    ...
    do {
        if (z_try_recv(handler, &sample) == _Z_RES_OK) {
        ...
        if (timeout_ms > 0) {
            size_t elapsed = z_clock_elapsed_ms(&start);
            if (elapsed >= timeout_ms) break;
            size_t remaining = timeout_ms - elapsed;
            size_t nap = (remaining < SLEEP_MS) ? remaining : SLEEP_MS;
            if (nap > 0) {
                z_sleep_ms(nap);
            }
        }
    ...
    assert(false && "expect_next_timeout(): timed out waiting for next sample");
}

static inline void expect_next(const z_loaned_fifo_handler_sample_t *handler, const char *expected) {
    expect_next_timeout(handler, expected, 0);
}
...
static void test_advanced_history(bool p2p) {
    ...
    expect_next_timeout(z_loan(handler), "2", SLEEP_MS);
...
static void test_advanced_retransmission(void) {
    ...
    const size_t SLEEP_MS = 3000;
    const size_t RECONNECT_SLEEP_MS = 5000;
    ...
    expect_next_timeout(z_loan(handler), "1", SLEEP_MS);
    expect_empty(z_loan(handler));
...
static void test_advanced_retransmission_periodic(void) {
    ...
    const size_t SLEEP_MS = 3000;
    const size_t RECONNECT_SLEEP_MS = 8000;
    const uint64_t PERIODIC_QUERY_MS = 1000;
    ...
    sub_opts.recovery.last_sample_miss_detection.periodic_queries_period_ms = PERIODIC_QUERY_MS;
    z_owned_closure_sample_t closure;
    z_owned_fifo_handler_sample_t handler;
    ASSERT_OK(z_fifo_channel_sample_new(&closure, &handler, 10));
    ASSERT_OK(ze_declare_advanced_subscriber(z_loan(s2), &sub, z_loan(k), z_move(closure), &sub_opts));
    ...
    expect_next_timeout(z_loan(handler), "2", RECONNECT_SLEEP_MS);
    expect_next(z_loan(handler), "3");
    expect_next(z_loan(handler), "4");
    expect_empty(z_loan(handler));
...
static void miss_handler(const ze_miss_t *miss, void *arg) {
    ...
    int idx = atomic_fetch_add_explicit(&ctx->event_count, 1, memory_order_acq_rel);
    assert(idx < MAX_MISS_EVENTS && "miss_ctx_t overflowed: increase MAX_MISS_EVENTS");

    ctx->nbs[idx] = miss->nb;
    ctx->sources[idx] = miss->source;
    atomic_fetch_add_explicit(&ctx->total_nb, ctx->nbs[idx], memory_order_acq_rel);
}
...
static void test_advanced_sample_miss(void) {
    ...
    expect_next_timeout(z_loan(handler), "3", SLEEP_MS);
    expect_empty(z_loan(handler));
...
static void test_advanced_retransmission_sample_miss(void) {
    ...
    expect_next_timeout(z_loan(handler), "4", SLEEP_MS);
    expect_next(z_loan(handler), "5");
    expect_empty(z_loan(handler));
...
static void test_advanced_retransmission_heartbeat(void) {
    ...
    const size_t SLEEP_MS = 3000;
    const size_t RECONNECT_SLEEP_MS = 5000;
    const uint64_t HEARTBEAT_PERIOD_MS = 4000;
    ...
    pub_opts.sample_miss_detection.heartbeat_period_ms = HEARTBEAT_PERIOD_MS;
    ASSERT_OK(ze_declare_advanced_publisher(z_loan(s1), &pub, z_loan(k), &pub_opts));
