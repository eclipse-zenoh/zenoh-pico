...
#define _ZP_VECTOR_TEMPLATE_ELEM_TYPE _z_bytes_t
#define _ZP_VECTOR_TEMPLATE_NAME _z_hbytes_vec
#define _ZP_VECTOR_TEMPLATE_ELEM_DESTROY_FN(elem) _z_bytes_clear(elem)
#define _ZP_VECTOR_TEMPLATE_ALLOC_FN(size) z_malloc(size)
#define _ZP_VECTOR_TEMPLATE_REALLOC_FN(ptr, size) z_realloc(ptr, size)
#define _ZP_VECTOR_TEMPLATE_FREE_FN(ptr) z_free(ptr)
#include "zenoh-pico/collections/vector_template.h"

// Storage owning every bytes buffer produced by gen_view_bytes.
// The generated message bodies now hold non-owning views (_z_bytes_view_t) into this storage; it shares the same
// lifetime as STRING_STORAGE (every test that generates a message body also generates view encodings/bytes), so both
// are initialized and destroyed together by string_storage_init()/string_storage_destroy().
_z_hbytes_vec_t BYTES_STORAGE;

static inline void string_storage_init(void) {
    _z_hstr_vec_init(&STRING_STORAGE);
    _z_hbytes_vec_init(&BYTES_STORAGE);
}
static inline void string_storage_destroy(void) {
    _z_hstr_vec_destroy(&STRING_STORAGE);
    _z_hbytes_vec_destroy(&BYTES_STORAGE);
}
...
// Generates a non-owning view over freshly generated bytes; the owning bytes are stored in BYTES_STORAGE so the
// underlying buffer stays valid for the whole test (mirrors gen_string/STRING_STORAGE). The returned view is a
// by-value copy of the bytes struct aliasing the same heap buffer, so it remains valid even if BYTES_STORAGE
// reallocates. Requires string_storage_init/destroy (which also manage BYTES_STORAGE).
_z_bytes_view_t gen_view_bytes(size_t len) {
    _z_bytes_t b = gen_bytes(len);
    _z_bytes_view_t view = _z_bytes_view_from_bytes(&b);
    // Storage takes ownership of `b` (and thus frees the buffer on destroy); the view only aliases it.
    assert(_z_hbytes_vec_push_back(&BYTES_STORAGE, &b) == true);
    return view;
}
...
_z_encoding_view_t gen_view_encoding(void) {
...
void payload_field(void) {
    ...
    assert(res == _Z_RES_OK);
    printf("   ");
    assert_eq_bytes(&e_pld, _z_bytes_view_deref(&d_pld));
    printf("\n");
...
_z_wireexpr_t gen_wireexpr(void) {
    ...
    if (is_numerical == true) {
        ...
        key._suffix = _z_string_view_from_string(&suffix);
    }
    return key;
}
...
_z_msg_query_t gen_query(void) {
    return (_z_msg_query_t){
        ...
        ._ext_attachment = gen_bool() ? gen_view_bytes(1 + gen_uint8()) : _z_bytes_view_null(),
    };
}
...
_z_msg_err_t gen_err(void) {
    ...
    return (_z_msg_err_t){
        ...
        ._payload = gen_view_bytes(len),  // Hangs if 0
    };
}
...
_z_transport_message_t gen_init(void) {
    if (gen_bool()) {
        ...
        _z_slice_t cookie = gen_slice(16);
        return _z_t_msg_make_init_ack(_z_whatami_from_uint8((gen_uint8() % 3)), gen_zid(), &cookie);
    }
}
