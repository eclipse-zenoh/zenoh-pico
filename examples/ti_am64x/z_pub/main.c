 ...
 *
 * z_pub — zenoh-pico publisher example for TI AM64x Cortex-R5F.
 *
 ...
 *
 * Build prerequisites:
 *   1. Generate SysConfig files for your board into examples/ti_am64x/syscfg/
 ...
 *            -B build/ti_am64x -S .
 *   3. cmake --build build/ti_am64x --target z_pub
 */
...
/* zenoh-pico pool allocator stats (ti_am64x-specific) */
...
static void z_pub_task(void *arg) {
    ...
    for (uint32_t idx = 0; ; idx++) {
        snprintf(buf, sizeof(buf), "[%4u] Hello from TI AM64x R5F!", idx);
