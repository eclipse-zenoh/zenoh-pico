...
_z_value_t _z_value_null(void) { return (_z_value_t){.payload = _z_slice_empty(), .encoding = _z_encoding_null()}; }
...
int8_t _z_attachment_get_seeker(_z_slice_t key, _z_slice_t value, void *ctx) {
    struct _z_seeker_t *seeker = (struct _z_seeker_t *)ctx;
    _z_slice_t seeked = seeker->key;
    if (key.len == seeked.len && memcmp(key.start, seeked.start, seeked.len)) {
        seeker->value = (_z_slice_t){.start = value.start, .len = value.len, ._is_alloc = false};
        return 1;
    }
    return 0;
}
...
int8_t _z_attachment_estimate_length_body(_z_slice_t key, _z_slice_t value, void *ctx) {
...
int8_t _z_encoded_attachment_iteration_driver(const void *this_, z_attachment_iter_body_t body, void *ctx) {
    _z_zbuf_t data = _z_slice_as_zbuf(*(_z_slice_t *)this_);
    while (_z_zbuf_can_read(&data)) {
        _z_slice_t key = _z_slice_empty();
        _z_slice_t value = _z_slice_empty();
        _z_slice_decode(&key, &data);
        _z_slice_decode(&value, &data);
...
z_attachment_t _z_encoded_as_attachment(const _z_owned_encoded_attachment_t *att) {
    if (att->is_encoded) {
        ...
        z_bytes_t *att_data = (_z_slice_t *)z_malloc(sizeof(_z_slice_t));
        if (att_data == NULL) {
...
void _z_attachment_drop(z_attachment_t *att) {
    if (att->iteration_driver == _z_encoded_attachment_iteration_driver) {
        _z_slice_clear((z_bytes_t *)att->data);
