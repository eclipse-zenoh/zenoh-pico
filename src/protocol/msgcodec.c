...
void _z_attachment_decode_na(_z_zbuf_t *zbf, uint8_t header, _z_attachment_p_result_t *r) {
    ...
    if (_Z_HAS_FLAG(r->_value->_header, _Z_FLAG_T_Z) != 0) {
...
void _z_data_info_decode_na(_z_zbuf_t *zbf, _z_data_info_result_t *r) {
    ...
    // WARNING: we do not support sliced content in zenoh-pico.
    //          Return error in case the payload is sliced.
    if (_Z_HAS_FLAG(r->_value._flags, _Z_DATA_INFO_SLICED) != 0) {
        r->_tag = _Z_ERR_PARSE_PAYLOAD;
        return;
    }

    if (_Z_HAS_FLAG(r->_value._flags, _Z_DATA_INFO_KIND) != 0) {
    ...
    if (_Z_HAS_FLAG(r->_value._flags, _Z_DATA_INFO_ENC) != 0) {
    ...
    if (_Z_HAS_FLAG(r->_value._flags, _Z_DATA_INFO_TSTAMP) != 0) {
    ...
    if (_Z_HAS_FLAG(r->_value._flags, _Z_DATA_INFO_RTR_SN) != 0) {
...
void _z_join_decode_na(_z_zbuf_t *zbf, uint8_t header, _z_join_result_t *r) {
    ...
    if (_Z_HAS_FLAG(r->_value._options, _Z_OPT_JOIN_QOS) != 0) {
