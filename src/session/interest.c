...
static z_result_t _z_interest_send_decl_resource(_z_session_t *zn, _z_optional_id_t interest_id,
                                                 const _z_peer_mask_bitset_t *opt_peer, const _z_keyexpr_t *restr_key) {
    ...
    while (xs != NULL) {
        ...
        if (restr_key == NULL || _z_keyexpr_intersects(restr_key, &res->_key)) {
            ...
            z_result_t ret = _z_send_n_msg(zn, &n_msg, Z_RELIABILITY_RELIABLE, Z_CONGESTION_CONTROL_BLOCK, opt_peer);
            if (ret != _Z_RES_OK) {
...
static z_result_t _z_interest_send_decl_subscriber(_z_session_t *zn, _z_optional_id_t interest_id,
    ...
    while (xs != NULL) {
        ...
        if (restr_key == NULL || _z_keyexpr_intersects(restr_key, &_Z_RC_IN_VAL(sub)->_key._inner)) {
            ...
            _z_network_message_t n_msg;
            _z_n_msg_make_declare(&n_msg, declaration, interest_id);
            z_result_t ret = _z_send_n_msg(zn, &n_msg, Z_RELIABILITY_RELIABLE, Z_CONGESTION_CONTROL_BLOCK, opt_peers);
            if (ret != _Z_RES_OK) {
...
static z_result_t _z_interest_send_decl_token(_z_session_t *zn, _z_optional_id_t interest_id,
                                              const _z_peer_mask_bitset_t *opt_peers, const _z_keyexpr_t *restr_key) {
    _Z_RETURN_IF_ERR(_z_session_mutex_lock_if_open(zn));
...
z_result_t _z_interest_push_declarations_to_peer(_z_session_t *zn, size_t peer_id) {
    ...
    _z_peer_mask_bitset_t peer_bitmask = _z_peer_mask_bitset_make_from_single_peer(peer_id);
    _Z_RETURN_IF_ERR(_z_interest_send_decl_resource(zn, interest_id, &peer_bitmask, NULL));
    _Z_RETURN_IF_ERR(_z_interest_send_decl_subscriber(zn, interest_id, &peer_bitmask, NULL));
    _Z_RETURN_IF_ERR(_z_interest_send_decl_queryable(zn, interest_id, &peer_bitmask, NULL));
    _Z_RETURN_IF_ERR(_z_interest_send_decl_token(zn, interest_id, &peer_bitmask, NULL));
    _Z_RETURN_IF_ERR(_z_interest_send_declare_final(zn, interest_id, &peer_bitmask));
    return _Z_RES_OK;
}
...
z_result_t _z_interest_process_interest(_z_session_t *zn, const _z_wireexpr_t *wireexpr, uint32_t id, uint8_t flags,
    ...
    if (_Z_HAS_FLAG(flags, _Z_INTEREST_FLAG_CURRENT)) {
        ...
        if (ret == _Z_RES_OK && _Z_HAS_FLAG(flags, _Z_INTEREST_FLAG_KEYEXPRS)) {
            ...
            ret = _z_interest_send_decl_resource(zn, interest_id, &peer_mask, restr_key_opt);
        }
        if (ret == _Z_RES_OK && _Z_HAS_FLAG(flags, _Z_INTEREST_FLAG_SUBSCRIBERS)) {
            _Z_DEBUG("Sending declare subscribers");
            ret = _z_interest_send_decl_subscriber(zn, interest_id, &peer_mask, restr_key_opt);
        }
        if (ret == _Z_RES_OK && _Z_HAS_FLAG(flags, _Z_INTEREST_FLAG_QUERYABLES)) {
            _Z_DEBUG("Sending declare queryables");
            ret = _z_interest_send_decl_queryable(zn, interest_id, &peer_mask, restr_key_opt);
        }
        if (ret == _Z_RES_OK && _Z_HAS_FLAG(flags, _Z_INTEREST_FLAG_TOKENS)) {
            _Z_DEBUG("Sending declare tokens");
            ret = _z_interest_send_decl_token(zn, interest_id, &peer_mask, restr_key_opt);
        }
        // Send final declare
        _Z_SET_IF_OK(ret, _z_interest_send_declare_final(zn, interest_id, &peer_mask));
    }
    return ret;
}
