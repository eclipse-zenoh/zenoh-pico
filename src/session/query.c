...
#if Z_FEATURE_LOGGING == 1
static size_t _z_pending_reply_count(const _z_pending_reply_slist_t *head) {
    size_t count = 0;
    const _z_pending_reply_slist_t *node = head;
    while (node != NULL) {
        count++;
        node = _z_pending_reply_slist_next((_z_pending_reply_slist_t *)node);
    }
    return count;
}
#endif
...
void _z_pending_query_clear(_z_pending_query_t *pen_qry) {
    ...
    pen_qry->_allowed_destination = zp_locality_default();
}
...
#if Z_FEATURE_LOGGING == 1
    _Z_DEBUG("reply_partial id=%jd pending=%zu drop=%d", (intmax_t)id,
             _z_pending_reply_count(pen_qry->_pending_replies), drop_flag);
#endif
    _z_session_mutex_unlock(zn);
...
z_result_t _z_trigger_query_reply_final(_z_session_t *zn, _z_zint_t id, bool finalize) {
    ...
    _Z_DEBUG("trigger_reply_final id=%jd finalize=%d", (intmax_t)id, finalize);
#if Z_FEATURE_LOGGING == 1
    _Z_DEBUG("reply_final id=%jd pending_before=%zu", (intmax_t)id, _z_pending_reply_count(pen_qry->_pending_replies));
#endif
    bool should_finalize = finalize || !zp_locality_allows_remote(pen_qry->_allowed_destination);
