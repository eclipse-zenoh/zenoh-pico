...
_z_hello_array_t __z_scout_loop(const _z_wbuf_t *wbf, const char *locator, unsigned long period, int exit_on_first) {
    ...
    if (r_scout._tag == _Z_RES_ERR) return ls;
    ...
    while (z_time_elapsed_ms(&start) < period) {
        ...
        if (ls._len > 0 && exit_on_first) break;
    }
