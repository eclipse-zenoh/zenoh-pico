...
z_result_t _z_trigger_queryables(_z_transport_common_t *transport, _z_msg_query_t *msgq, _z_wireexpr_t *q_key,
    ...
    _Z_SET_IF_OK(ret,
                 _z_query_move_data(_Z_RC_IN_VAL(&query), &msgq->_ext_value, &qle_infos.ke, &msgq->_parameters,
                                    &transport->_session, qid, &msgq->_ext_attachment, msgq->_anyke, &msgq->_ext_info));
