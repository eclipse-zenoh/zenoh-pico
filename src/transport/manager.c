...
static z_result_t _z_new_transport_peer(_z_transport_t *zt, _z_string_t *locator, _z_id_t *local_zid, bool is_open) {
    z_result_t ret = _Z_RES_OK;
    // Init link
    _z_link_t zl;
    memset(&zl, 0, sizeof(_z_link_t));
    ...
    switch (zl._cap._transport) {
        case Z_LINK_CAP_TRANSPORT_UNICAST: {
            _z_transport_unicast_establish_param_t tp_param;
            ret = _z_unicast_open_peer(&tp_param, &zl, local_zid, is_open);
            if (ret != _Z_RES_OK) {
                _z_link_clear(&zl);
                return ret;
            }
...
z_result_t _z_new_transport(_z_transport_t *zt, _z_id_t *bs, _z_string_t *locator, z_whatami_t mode, bool is_open) {
    ...
    if (mode == Z_WHATAMI_CLIENT) {
        ...
        ret = _z_new_transport_peer(zt, locator, bs, is_open);
    }
