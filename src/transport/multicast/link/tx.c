...
int _z_multicast_send_z_msg(_z_session_t *zn, _z_zenoh_message_t *z_msg, z_reliability_t reliability,
    ...
    if (res == 0) {
        ...
        while (_z_wbuf_len(&fbf) > 0) {
            ...
            if (!is_first) sn = __unsafe_z_multicast_get_sn(ztm, reliability);
            is_first = 0;
