...
int8_t _z_multicast_send_z_msg(_z_session_t *zn, _z_zenoh_message_t *z_msg, _z_zint_t count,
    ...
    if (drop == false) {
        ...
        if (ret == _Z_RES_OK) {
            for (_z_zint_t i=0;i<count;i++) {
                ret = _z_zenoh_message_encode(&ztm->_wbuf, &z_msg[i]);  // Encode the zenoh message
                if(ret != _Z_RES_OK) {
                    break;
                }
            }
