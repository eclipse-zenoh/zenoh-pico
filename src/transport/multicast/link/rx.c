...
int _zn_multicast_handle_transport_message(_zn_transport_multicast_t *ztm, _zn_transport_message_t *t_msg, z_bytes_t *addr)
{
    ...
    case _ZN_MID_FRAGMENT:
    {
        ...
        if (_ZN_HAS_FLAG(t_msg->header, _ZN_FLAG_HDR_FRAGMENT_R))
        {
            // @TODO: amend once reliability is in place. For the time being only
            //        monothonic SNs are ensured
            if (_zn_sn_precedes(entry->sn_resolution_half, entry->sn_rx.reliable, t_msg->body.fragment.sn))
                entry->sn_rx.reliable = t_msg->body.fragment.sn;
            else
            {
                _z_wbuf_clear(&entry->dbuf_reliable);
                _Z_DEBUG("Reliable message dropped because it is out of order");
        ...
        {
            if (_zn_sn_precedes(entry->sn_resolution_half, entry->sn_rx.best_effort, t_msg->body.fragment.sn))
                entry->sn_rx.best_effort = t_msg->body.fragment.sn;
            else
            {
                _z_wbuf_clear(&entry->dbuf_best_effort);
                _Z_DEBUG("Best effort message dropped because it is out of order");
        ...
        _z_wbuf_t *dbuf = _ZN_HAS_FLAG(t_msg->header, _ZN_FLAG_HDR_FRAGMENT_R) ? &entry->dbuf_reliable : &entry->dbuf_best_effort;
