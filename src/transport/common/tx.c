...
void _z_wbuf_prepare(_z_wbuf_t *buf, uint8_t link_flow_capability) {
    _z_wbuf_reset(buf);
...
void _z_wbuf_finalize(_z_wbuf_t *buf, uint8_t link_flow_capability) {
    switch (link_flow_capability) {
        // Stream capable links
        case Z_LINK_CAP_FLOW_STREAM: {
...
z_result_t _z_send_n_msg(_z_session_t *zn, const _z_network_message_t *z_msg, z_reliability_t reliability,
                         z_congestion_control_t cong_ctrl, const _z_peer_mask_bitset_t *opt_peers) {
#if defined(Z_TEST_HOOKS)
    if (_z_send_n_msg_override != NULL) {
        bool handled = false;
        z_result_t override_ret = _z_send_n_msg_override(zn, z_msg, reliability, cong_ctrl, opt_peers, &handled);
        if (handled) {
            return override_ret;
        }
    }
    ...
    switch (zn->_tp._type) {
        case _Z_TRANSPORT_UNICAST_TYPE:
            return _z_transport_unicast_send_n_msg(&zn->_tp._transport._unicast, z_msg, reliability, cong_ctrl,
                                                   opt_peers);
        case _Z_TRANSPORT_MULTICAST_TYPE:
            return _z_transport_multicast_send_n_msg(&zn->_tp._transport._multicast, z_msg, reliability, cong_ctrl);
        case _Z_TRANSPORT_RAWETH_TYPE:
            return _z_transport_raweth_send_n_msg(&zn->_tp._transport._raweth, z_msg, reliability, cong_ctrl);
        default:
...
z_result_t _z_send_n_batch(_z_session_t *zn) {
    switch (zn->_tp._type) {
        case _Z_TRANSPORT_UNICAST_TYPE:
            return _z_transport_unicast_send_n_batch(&zn->_tp._transport._unicast);
        case _Z_TRANSPORT_MULTICAST_TYPE:
            return _z_transport_multicast_send_n_batch(&zn->_tp._transport._multicast);
        case _Z_TRANSPORT_RAWETH_TYPE:
            return _z_transport_raweth_send_n_batch(&zn->_tp._transport._raweth);
        default:
            _Z_ERROR_LOG(_Z_ERR_TRANSPORT_NOT_AVAILABLE);
            return _Z_ERR_TRANSPORT_NOT_AVAILABLE;
    }
