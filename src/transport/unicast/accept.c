...
static const _z_stream_ops_t *_zp_unicast_accept_stream_ops(const _z_link_t *link) {
    if (link == NULL) {
        return NULL;
    }
...
_z_fut_fn_result_t _zp_unicast_accept_task_fn(void *ctx, _z_executor_t *executor) {
    ...
    const _z_stream_ops_t *stream_ops = _zp_unicast_accept_stream_ops(ztu->_common._link);
    if (socket_ptr == NULL) {
        _Z_ERROR_LOG(_Z_ERR_INVALID);
        return _z_fut_fn_result_ready();
    }
    if (stream_ops == NULL) {
        _Z_INFO("No stream backend available for accept");
        return _z_fut_fn_result_ready();
    }

    _z_sys_net_socket_t listen_socket = *socket_ptr;
    _z_sys_net_socket_t con_socket = {0};
    z_result_t ret = _z_stream_accept(stream_ops, &listen_socket, &con_socket);
    if (ret != _Z_RES_OK) {
