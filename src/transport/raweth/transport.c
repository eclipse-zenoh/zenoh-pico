...
int8_t _z_raweth_transport_create(_z_transport_t *zt, _z_link_t *zl, _z_transport_multicast_establish_param_t *param) {
    ...
    ret = _z_mutex_init(&zt->_transport._raweth._mutex_tx);
    if (ret == _Z_RES_OK) {
        ret = _z_mutex_init(&zt->_transport._raweth._mutex_rx);
        if (ret == _Z_RES_OK) {
            ret = _z_mutex_init(&zt->_transport._raweth._mutex_peer);
            if (ret != _Z_RES_OK) {
                _z_mutex_free(&zt->_transport._raweth._mutex_tx);
                _z_mutex_free(&zt->_transport._raweth._mutex_rx);
            }
        } else {
            _z_mutex_free(&zt->_transport._raweth._mutex_tx);
        }
    ...
    if (ret == _Z_RES_OK) {
        ...
        zt->_transport._raweth._wbuf = _z_wbuf_make(mtu, false);
        zt->_transport._raweth._zbuf = _z_zbuf_make(Z_BATCH_MULTICAST_SIZE);

        // Clean up the buffers if one of them failed to be allocated
        if ((_z_wbuf_capacity(&zt->_transport._raweth._wbuf) != mtu) ||
            (_z_zbuf_capacity(&zt->_transport._raweth._zbuf) != Z_BATCH_MULTICAST_SIZE)) {
            ret = _Z_ERR_SYSTEM_OUT_OF_MEMORY;

#if Z_FEATURE_MULTI_THREAD == 1
            _z_mutex_free(&zt->_transport._raweth._mutex_tx);
            _z_mutex_free(&zt->_transport._raweth._mutex_rx);
            _z_mutex_free(&zt->_transport._raweth._mutex_peer);
#endif  // Z_FEATURE_MULTI_THREAD == 1

            _z_wbuf_clear(&zt->_transport._raweth._wbuf);
            _z_zbuf_clear(&zt->_transport._raweth._zbuf);
        }
    }

    if (ret == _Z_RES_OK) {
        // Set default SN resolution
        zt->_transport._raweth._sn_res = _z_sn_max(param->_seq_num_res);

        // The initial SN at TX side
        zt->_transport._raweth._sn_tx_reliable = param->_initial_sn_tx._val._plain._reliable;
        zt->_transport._raweth._sn_tx_best_effort = param->_initial_sn_tx._val._plain._best_effort;

        // Initialize peer list
        zt->_transport._raweth._peers = _z_transport_peer_entry_list_new();

#if Z_FEATURE_MULTI_THREAD == 1
        // Tasks
        zt->_transport._raweth._read_task_running = false;
        zt->_transport._raweth._read_task = NULL;
        zt->_transport._raweth._lease_task_running = false;
        zt->_transport._raweth._lease_task = NULL;
#endif  // Z_FEATURE_MULTI_THREAD == 1

        zt->_transport._raweth._lease = Z_TRANSPORT_LEASE;

        // Notifiers
        zt->_transport._raweth._transmitted = false;

        // Transport link for raweth
        zt->_transport._raweth._link = *zl;
    }
...
int8_t _z_raweth_send_close(_z_transport_multicast_t *ztm, uint8_t reason, _Bool link_only) {
    ...
    ret = _z_raweth_send_t_msg(ztm, &cm);
    _z_t_msg_clear(&cm);
    return ret;
}

int8_t _z_raweth_transport_close(_z_transport_multicast_t *ztm, uint8_t reason) {
    return _z_raweth_send_close(ztm, reason, false);
}

void _z_raweth_transport_clear(_z_transport_t *zt) {
    _z_transport_multicast_t *ztm = &zt->_transport._raweth;
#if Z_FEATURE_MULTI_THREAD == 1
    // Clean up tasks
...
void _z_raweth_transport_clear(_z_transport_t *zt) { _ZP_UNUSED(zt); }
