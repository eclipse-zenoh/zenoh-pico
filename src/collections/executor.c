...
_z_executor_spin_result_t _z_executor_get_next_fut(_z_executor_t *executor, _z_fut_data_hmap_index_t *task_idx) {
    ...
    _z_sleeping_fut_data_t *sleeping_task_ptr = _z_sleeping_fut_data_pqueue_peek(&executor->_sleeping_tasks);
    if (sleeping_task_ptr != NULL) {
        ...
        if (zp_clock_elapsed_ms_since(&now, &wake_up_time) > 0) {
            ...
            _z_sleeping_fut_data_t t;
            _z_sleeping_fut_data_pqueue_pop(&executor->_sleeping_tasks, &t);
            if (_z_fut_data_hmap_index_deque_pop_front(&executor->_ready_tasks, task_idx)) {
                ...
                _z_fut_data_hmap_index_deque_push_back(&executor->_ready_tasks, &t._fut_idx);
                // Mark the sleeping task as ready since it's now
                // re-enqueued to the ready task queue and can be executed.
                _z_fut_data_hmap_node_at(&executor->_tasks, t._fut_idx)->val._status = _Z_FUT_STATUS_READY;
            } else {
...
_z_executor_spin_result_t _z_executor_spin(_z_executor_t *executor) {
    ...
    if (fn_result._status == _Z_FUT_STATUS_RUNNING) {
        ...
        fut_data->_status = _Z_FUT_STATUS_SLEEPING;
        _z_sleeping_fut_data_t sleeping_task;
        sleeping_task._fut_idx = fut_idx;
        sleeping_task._wake_up_time_ms = zp_clock_elapsed_ms_since(&fn_result._wake_up_time, &executor->_epoch);
        // can't fail since we have enough capacity for all tasks in the hashmap
        _z_sleeping_fut_data_pqueue_push(&executor->_sleeping_tasks, &sleeping_task);
    } else {
