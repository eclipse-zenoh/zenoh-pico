...
/*-------- Bytes --------*/
_Bool _zz_bytes_check(const _zz_bytes_t *bytes) { return !_zz_bytes_is_empty(bytes); }
...
size_t _zz_bytes_num_slices(const _zz_bytes_t *bs) { return _z_arc_slice_svec_len(&bs->_slices); }
...
int8_t _zz_bytes_to_slice(const _zz_bytes_t *bytes, _z_slice_t *s) {
    ...
    *s = _z_slice_make(len);
    ...
    for (size_t i = 0; i < _zz_bytes_num_slices(bytes); ++i) {
        // Recopy data
        _z_arc_slice_t *arc_s = _zz_bytes_get_slice(bytes, i);
...
int8_t _zz_bytes_serialize_from_pair(_zz_bytes_t *out, _zz_bytes_t *first, _zz_bytes_t *second) {
    int8_t res = _zz_bytes_append(out, first);
    if (res != _Z_RES_OK) {
        _zz_bytes_drop(out);
        _zz_bytes_drop(first);
        return res;
    }
    res = _zz_bytes_append(out, second);
    if (res != _Z_RES_OK) {
        _zz_bytes_drop(out);
...
int8_t _zz_bytes_to_uint8(const _zz_bytes_t *bs, uint8_t *val) {
    *val = 0;
    return _zz_bytes_to_buf(bs, val, sizeof(uint8_t)) == sizeof(uint8_t) ? _Z_RES_OK : _Z_ERR_DID_NOT_READ;
}

// FIXME: int16+ endianness, Issue #420
int8_t _zz_bytes_to_uint16(const _zz_bytes_t *bs, uint16_t *val) {
    *val = 0;
    return _zz_bytes_to_buf(bs, (uint8_t *)val, sizeof(uint16_t)) == sizeof(uint16_t) ? _Z_RES_OK : _Z_ERR_DID_NOT_READ;
}

int8_t _zz_bytes_to_uint32(const _zz_bytes_t *bs, uint32_t *val) {
    *val = 0;
    return _zz_bytes_to_buf(bs, (uint8_t *)val, sizeof(uint32_t)) == sizeof(uint32_t) ? _Z_RES_OK : _Z_ERR_DID_NOT_READ;
}

int8_t _zz_bytes_to_uint64(const _zz_bytes_t *bs, uint64_t *val) {
    *val = 0;
    return _zz_bytes_to_buf(bs, (uint8_t *)val, sizeof(uint64_t)) == sizeof(uint64_t) ? _Z_RES_OK : _Z_ERR_DID_NOT_READ;
}

int8_t _zz_bytes_to_float(const _zz_bytes_t *bs, float *val) {
    *val = 0;
    return _zz_bytes_to_buf(bs, (uint8_t *)val, sizeof(float)) == sizeof(float) ? _Z_RES_OK : _Z_ERR_DID_NOT_READ;
}

int8_t _zz_bytes_to_double(const _zz_bytes_t *bs, double *val) {
    *val = 0;
    return _zz_bytes_to_buf(bs, (uint8_t *)val, sizeof(double)) == sizeof(double) ? _Z_RES_OK : _Z_ERR_DID_NOT_READ;
}

int8_t _zz_bytes_from_uint8(_zz_bytes_t *b, uint8_t val) { return _zz_bytes_from_buf(b, &val, sizeof(val)); }

int8_t _zz_bytes_from_uint16(_zz_bytes_t *b, uint16_t val) {
    return _zz_bytes_from_buf(b, (uint8_t *)&val, sizeof(val));
}
...
int8_t _zz_bytes_from_float(_zz_bytes_t *b, float val) { return _zz_bytes_from_buf(b, (uint8_t *)&val, sizeof(val)); }

int8_t _zz_bytes_from_double(_zz_bytes_t *b, double val) { return _zz_bytes_from_buf(b, (uint8_t *)&val, sizeof(val)); }

_z_slice_t _zz_bytes_try_get_contiguous(const _zz_bytes_t *bs) {
    if (_zz_bytes_num_slices(bs) == 1) {
        _z_arc_slice_t *arc_s = _zz_bytes_get_slice(bs, 0);
    ...
    switch (origin) {
        ...
        case SEEK_CUR: {
            if (offset >= 0)
                ...
                return _zz_bytes_reader_seek_backward(reader, -offset);
        }
        case SEEK_END: {
...
int64_t _zz_bytes_reader_tell(const _zz_bytes_reader_t *reader) { return reader->byte_idx; }
...
_zz_bytes_iterator_t _zz_bytes_get_iterator(const _zz_bytes_t *bytes) {
    return (_zz_bytes_iterator_t){._reader = _zz_bytes_get_reader(bytes)};
}

_Bool _zz_bytes_iterator_next(_zz_bytes_iterator_t *iter, _zz_bytes_t *b) {
    return _zz_bytes_reader_read_next(&iter->_reader, b) == _Z_RES_OK;
}
