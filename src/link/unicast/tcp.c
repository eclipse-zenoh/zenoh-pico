...
z_result_t _z_f_link_open_tcp(_z_link_t *zl) {
    ...
    return _z_stream_open(ops, &zl->_socket._tcp._sock, zl->_socket._tcp._rep, tout);
}
...
z_result_t _z_f_link_listen_tcp(_z_link_t *zl) {
    ...
    return _z_stream_listen(ops, &zl->_socket._tcp._sock, zl->_socket._tcp._rep);
}

void _z_f_link_close_tcp(_z_link_t *zl) {
    const _z_stream_ops_t *ops = _z_default_stream_ops();
    if (ops != NULL) {
        _z_stream_close(ops, &zl->_socket._tcp._sock);
    }
}
...
void _z_f_link_free_tcp(_z_link_t *zl) {
    const _z_stream_ops_t *ops = _z_default_stream_ops();
    if (ops != NULL) {
        _z_stream_endpoint_clear(ops, &zl->_socket._tcp._rep);
    }
}
...
size_t _z_f_link_write_tcp(const _z_link_t *zl, const uint8_t *ptr, size_t len, _z_sys_net_socket_t *socket) {
    ...
    if (socket != NULL) {
        return _z_stream_write(ops, *socket, ptr, len);
    } else {
        return _z_stream_write(ops, zl->_socket._tcp._sock, ptr, len);
    }
}
...
size_t _z_f_link_read_tcp(const _z_link_t *zl, uint8_t *ptr, size_t len, _z_slice_t *addr) {
    ...
    return _z_stream_read(ops, zl->_socket._tcp._sock, ptr, len);
}
...
size_t _z_f_link_read_exact_tcp(const _z_link_t *zl, uint8_t *ptr, size_t len, _z_slice_t *addr,
    ...
    if (socket != NULL) {
        return _z_stream_read_exact(ops, *socket, ptr, len);
    } else {
        return _z_stream_read_exact(ops, zl->_socket._tcp._sock, ptr, len);
    }
}
...
z_result_t _z_new_peer_tcp(_z_endpoint_t *endpoint, _z_sys_net_socket_t *socket) {
    ...
    const _z_stream_ops_t *ops = _z_default_stream_ops();
    z_result_t ret = _z_stream_endpoint_init_from_address(ops, &sys_endpoint, &endpoint->_locator._address);

    if (ret != _Z_RES_OK) {
        if (ops != NULL) {
            _z_stream_endpoint_clear(ops, &sys_endpoint);
        }
        return ret;
    }

    ret = _z_stream_open(ops, socket, sys_endpoint, Z_CONFIG_SOCKET_TIMEOUT);
    if (ops != NULL) {
        _z_stream_endpoint_clear(ops, &sys_endpoint);
    }
    return ret;
...
z_result_t _z_new_link_tcp(_z_link_t *zl, _z_endpoint_t *endpoint) {
    ...
    z_result_t ret = _z_stream_endpoint_init_from_address(ops, &zl->_socket._tcp._rep, &endpoint->_locator._address);
