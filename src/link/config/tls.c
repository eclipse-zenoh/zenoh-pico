...
int8_t _z_tls_config_from_strn(_z_str_intmap_t *strint, const char *s, size_t n) {
    TLS_CONFIG_MAPPING_BUILD

    return _z_str_intmap_from_strn(strint, s, argc, args, n);
}

int8_t _z_tls_config_from_str(_z_str_intmap_t *strint, const char *s) {
    return _z_tls_config_from_strn(strint, s, strlen(s));
}
#endif
