...
char *_z_stream_address_parse_host(const _z_string_t *address) { return _z_endpoint_parse_host(address); }

z_result_t _z_stream_address_valid(const _z_string_t *address) {
    char *host = _z_stream_address_parse_host(address);
...
z_result_t _z_stream_endpoint_init_from_address(const _z_stream_ops_t *ops, _z_sys_net_endpoint_t *ep,
    ...
    if (ops == NULL) {
        ...
        ret = _z_stream_endpoint_init(ops, ep, host, port);
    }
