...
z_sample_ring_t z_sample_ring_make(size_t capacity) {
    z_sample_ring_t ring;
    ring._ring = _z_sample_ring_make(capacity);
...
void z_sample_ring_free(z_sample_ring_t **r) {
    z_sample_ring_t *ptr = *r;
    z_sample_ring_clear(ptr);
    zp_free(ptr);
    *r = NULL;
}

void z_sample_ring_push(z_sample_ring_t *r, const z_sample_t *s) {
    ...
    zp_mutex_lock(&r->_mutex);
#endif
    _z_sample_ring_push_force_drop(&r->_ring, os);
#if Z_FEATURE_MULTI_THREAD == 1
    zp_mutex_unlock(&r->_mutex);
#endif
}
...
z_owned_sample_t *z_sample_ring_pull(z_sample_ring_t *r) {
    ...
    zp_mutex_lock(&r->_mutex);
#endif
    z_owned_sample_t *ret = _z_sample_ring_pull(&r->_ring);
#if Z_FEATURE_MULTI_THREAD == 1
    zp_mutex_unlock(&r->_mutex);
#endif
    return ret;
