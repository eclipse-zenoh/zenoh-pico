...
_z_subscriber_t *_z_declare_subscriber(_z_session_t *zn, _z_keyexpr_t keyexpr, _z_subinfo_t sub_info,
    ...
    _z_subscription_sptr_t *sp_s =
        _z_register_subscription(zn, _Z_RESOURCE_IS_LOCAL, &s);  // This a pointer to the entry stored at session-level.
...
int8_t _z_query(_z_session_t *zn, _z_keyexpr_t keyexpr, const char *parameters, const z_query_target_t target,
                const z_consolidation_mode_t consolidation, _z_value_t query_body, _z_reply_handler_t callback,
                void *arg_call, _z_drop_handler_t dropper, void *arg_drop) {
    int8_t ret = _Z_RES_OK;
    ...
    if (ret == _Z_RES_OK) {
        _z_zenoh_message_t z_msg =
            _z_msg_make_query(keyexpr, pq->_parameters, pq->_id, pq->_target, pq->_consolidation, query_body);
