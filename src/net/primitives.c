...
z_result_t _z_write(_z_session_t *zn, const _z_keyexpr_t *keyexpr, const _z_bytes_t *payload,
    ...
    if (zp_locality_allows_local(allowed_destination)) {
    ...
    if (!zp_locality_allows_remote(allowed_destination)) {
        return _Z_RES_OK;
    }
...
static bool _z_session_has_remote_peer(const _z_session_t *zn) {
    switch (zn->_tp._type) {
        case _Z_TRANSPORT_UNICAST_TYPE: {
            const _z_transport_unicast_t *tp = &zn->_tp._transport._unicast;
            if (tp->_common._link != NULL) {
                _Z_DEBUG("remote_peer: link present for mode=%d", zn->_mode);
                return true;
            }
            if (zn->_mode == Z_WHATAMI_CLIENT) {
                _Z_DEBUG("remote_peer: client mode without link");
                return true;
            }
            bool has_peer = !_z_transport_peer_unicast_slist_is_empty(tp->_peers);
            _Z_DEBUG("remote_peer: peers=%d", has_peer);
            return has_peer;
        }
        case _Z_TRANSPORT_MULTICAST_TYPE: {
            const _z_transport_multicast_t *tp = &zn->_tp._transport._multicast;
            if (tp->_common._link != NULL) {
                _Z_DEBUG("remote_peer multicast: link present");
                return true;
            }
            bool has_peer = !_z_transport_peer_multicast_slist_is_empty(tp->_peers);
            _Z_DEBUG("remote_peer multicast: peers=%d", has_peer);
            return has_peer;
        }
        case _Z_TRANSPORT_RAWETH_TYPE: {
            const _z_transport_multicast_t *tp = &zn->_tp._transport._raweth;
            if (tp->_common._link != NULL) {
                _Z_DEBUG("remote_peer raweth: link present");
                return true;
            }
            bool has_peer = !_z_transport_peer_multicast_slist_is_empty(tp->_peers);
            _Z_DEBUG("remote_peer raweth: peers=%d", has_peer);
            return has_peer;
        }
        default:
            return false;
...
z_result_t _z_query(_z_session_t *zn, const _z_keyexpr_t *keyexpr, const char *parameters, size_t parameters_len,
    ...
    bool allow_local = zp_locality_allows_local(allowed_destination);
    bool allow_remote = zp_locality_allows_remote(allowed_destination);
    ...
    bool remote_possible = allow_remote && _z_session_has_remote_peer(zn);
    _Z_DEBUG("_z_query: handled_locally=%d remote_possible=%d", handled_locally, remote_possible);

    if (handled_locally && remote_possible) {
        _Z_RETURN_IF_ERR(_z_trigger_query_reply_final(zn, pq->_id, false));
    }

    if (!remote_possible) {
        if (!handled_locally || !_z_session_deliver_reply_final_locally(zn, pq->_id, false)) {
            _Z_RETURN_IF_ERR(_z_trigger_query_reply_final(zn, pq->_id, true));
        }
        return _Z_RES_OK;
    }
