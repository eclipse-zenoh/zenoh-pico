...
typedef z_result_t (*_z_open_connect_fn)(_z_session_rc_t *zs, _z_string_t *locator, const _z_id_t *zid,
                                         const _z_config_t *config, void *ctx);

typedef struct {
    const _z_string_t *_preferred_locator;
    _z_open_connect_fn _open_connect;
    void *_open_connect_ctx;
} _z_open_options_t;
...
static z_result_t _z_open_connect_default(_z_session_rc_t *zs, _z_string_t *locator, const _z_id_t *zid,
                                          const _z_config_t *config, void *ctx) {
    _ZP_UNUSED(ctx);
...
static void _z_open_options_default(_z_open_options_t *opts) {
    ...
    opts->_open_connect = _z_open_connect_default;
    opts->_open_connect_ctx = NULL;
}
...
static z_result_t _z_open_connect_locator(_z_session_rc_t *zn, _z_pending_peers_t *pending_peers, const _z_id_t *zid,
    ...
    _z_open_connect_fn open_connect = (opts != NULL) ? opts->_open_connect : NULL;
    void *open_connect_ctx = (opts != NULL) ? opts->_open_connect_ctx : NULL;
    if (open_connect == NULL) {
        open_connect = _z_open_connect_default;
    }
    ...
    while (!out->transport_opened) {
        ...
        for (size_t i = 0; i < connect_len; i++) {
            ...
            ret = open_connect(zn, locator, zid, config, open_connect_ctx);
            if (ret == _Z_RES_OK) {
...
z_result_t _z_open_locators_peer(_z_session_rc_t *zn, _z_string_t *listen_locator,
    ...
    if (!transport_opened && (connect_len > 0)) {
        ...
        _Z_CLEAN_RETURN_IF_ERR(_z_open_connect_locator(zn, &pending_peers, zid, config, connect_timeout_ms,
                                                       connect_exit_on_failure, NULL, &connect_result),
                               _z_pending_peers_clear(&pending_peers));
...
z_result_t _z_open_locators(_z_session_rc_t *zn, const _z_string_svec_t *listen_locators,
    ...
    size_t listen_len = _z_string_svec_len(listen_locators);
    size_t connect_len = _z_string_svec_len(connect_locators);
    bool has_preferred_locator = (mode == Z_WHATAMI_CLIENT) && _z_open_has_preferred_locator(opts);

    if ((listen_len == 0) && (connect_len == 0) && !has_preferred_locator) {
...
static z_result_t _z_open_common(_z_session_rc_t *zn, _z_config_t *config, const _z_id_t *zid,
    ...
    bool fallback_to_scout = true;
    if ((mode == Z_WHATAMI_CLIENT) && _z_open_has_preferred_locator(opts)) {
        ret = _z_open_locators(zn, &listen_locators, &connect_locators, zid, config, mode, opts);
        fallback_to_scout = (ret != _Z_RES_OK);
    }
    if (!fallback_to_scout) {
        _z_string_svec_clear(&listen_locators);
        _z_string_svec_clear(&connect_locators);
        return ret;
    }
...
#if Z_FEATURE_AUTO_RECONNECT == 1
typedef struct {
    _z_transport_unicast_t *_target;
} _z_client_reopen_unicast_ctx_t;

static z_result_t _z_client_reopen_unicast_connection(_z_session_rc_t *zs, _z_string_t *locator, const _z_id_t *zid,
                                                      const _z_config_t *config, void *ctx_arg) {
    _z_client_reopen_unicast_ctx_t *ctx = (_z_client_reopen_unicast_ctx_t *)ctx_arg;
    _z_session_t *s = _Z_RC_IN_VAL(zs);
    _z_transport_t next = {0};

    z_result_t ret = _z_new_transport(&next, zid, locator, Z_WHATAMI_CLIENT, _Z_PEER_OP_OPEN, config, &s->_runtime);
    if (ret != _Z_RES_OK) {
        _z_transport_clear(&next);
        return ret;
    }
    if (next._type != _Z_TRANSPORT_UNICAST_TYPE) {
        _z_transport_clear(&next);
        return _Z_ERR_CONFIG_UNSUPPORTED_CLIENT_MULTICAST;
    }

    _z_transport_get_common(&next)->_session = _z_session_rc_clone_as_weak(zs);
    _z_transport_get_common(&next)->_state = _Z_TRANSPORT_STATE_OPEN;
    _z_unicast_transport_replace_connection(ctx->_target, &next._transport._unicast);
...
static z_result_t _z_client_reopen_unicast(_z_session_rc_t *zs) {
    _z_session_t *s = _Z_RC_IN_VAL(zs);
    _z_client_reopen_unicast_ctx_t ctx = {0};
    ctx._target = &s->_tp._transport._unicast;

    _z_open_options_t opts;
    _z_open_options_default(&opts);
    opts._preferred_locator = &s->_last_connect_locator;
    opts._open_connect = _z_client_reopen_unicast_connection;
    opts._open_connect_ctx = &ctx;
...
_z_fut_fn_result_t _z_client_reopen_task_fn(void *ztc_arg, _z_executor_t *executor) {
    ...
    bool in_place_reconnect = (s->_mode == Z_WHATAMI_CLIENT) && (s->_tp._type == _Z_TRANSPORT_UNICAST_TYPE);
    ...
    if (!_z_network_message_slist_is_empty(s->_declaration_cache)) {
        ...
        while (iter != NULL) {
            ...
            if (ret != _Z_RES_OK) {
                ...
                _z_session_transport_mutex_lock(s);
                if ((s->_mode == Z_WHATAMI_CLIENT) && (s->_tp._type == _Z_TRANSPORT_UNICAST_TYPE)) {
