...
uint8_t z_random_u8(void)
{
    ...
    while (getrandom(&ret, sizeof(uint8_t), GRND_RANDOM) < 0);
...
uint16_t z_random_u16(void)
{
    uint16_t ret;
#if defined(ZENOH_LINUX)
    while (getrandom(&ret, sizeof(uint16_t), GRND_RANDOM) < 0);
#elif defined(ZENOH_MACOS)
    ret = z_random_u32();
#endif
...
uint32_t z_random_u32(void)
{
    uint32_t ret;
#if defined(ZENOH_LINUX)
    while (getrandom(&ret, sizeof(uint32_t), GRND_RANDOM) < 0);
#elif defined(ZENOH_MACOS)
    ret = arc4random();
#endif
...
uint64_t z_random_u64(void)
{
    uint64_t ret = 0;
#if defined(ZENOH_LINUX)
    while (getrandom(&ret, sizeof(uint64_t), GRND_RANDOM) < 0);
#elif defined(ZENOH_MACOS)
    ret |= z_random_u32();
    ret |= z_random_u32() << 8;
#endif
