...
#define RX_DMA_BUFFER_SIZE (_Z_SERIAL_MAX_COBS_BUF_SIZE * 2 + 2) // for 2 max sized COBS frames + something little extra
...
z_result_t _z_open_serial_from_dev(_z_sys_net_socket_t *sock, char *dev, uint32_t baudrate) {
	...
	tx_semaphore_create(&data_processing_semaphore, "Data Processing", 1); // get is 1 step signal ahead, 0-still processing, 1-ready to process new data
...
size_t _z_send_serial_internal(const _z_sys_net_socket_t sock, uint8_t header, const uint8_t *ptr, size_t len) {
    ...
    size_t ret = _z_serial_msg_serialize(raw_buf, _Z_SERIAL_MAX_COBS_BUF_SIZE, ptr, len, header, tmp_buf, _Z_SERIAL_MFS_SIZE);
...
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart, uint16_t offset) {
    zptxstm32_rx_event_cb(huart, offset);
}
