...
{
    __zn_tcp_addr_t *addr = (__zn_tcp_addr_t *)z_malloc(sizeof(__zn_tcp_addr_t));
...
{
    __zn_tcp_addr_t *addr = (__zn_tcp_addr_t *)addr_arg;
    z_free(addr);
}

void *_zn_open_tcp(void *raddr_arg, const clock_t tout)
{
    __zn_tcp_addr_t *raddr = (__zn_tcp_addr_t *)raddr_arg;
...
void * _zn_listen_tcp(void *laddr_arg, const clock_t tout)
{
    __zn_tcp_addr_t *laddr = (__zn_tcp_addr_t *)laddr_arg;
...
size_t _zn_send_tcp(void *sock_arg, const uint8_t *ptr, size_t len, void *raddr_arg)
{
    WiFiClient *sock = (WiFiClient *)sock_arg;
    __zn_tcp_addr_t *raddr = (__zn_tcp_addr_t *)raddr_arg;
...
{
    __zn_udp_addr_t *addr = (__zn_udp_addr_t *)z_malloc(sizeof(__zn_udp_addr_t));
    addr->_ipaddr.fromString(s_addr);
    addr->_port = atoi(port);
...
{
    __zn_udp_addr_t *addr = (__zn_udp_addr_t *)addr_arg;
    z_free(addr);
}
#endif
...
{
    __zn_udp_addr_t *raddr = (__zn_udp_addr_t *)raddr_arg;
...
void * _zn_listen_udp_unicast(void *laddr_arg, const clock_t tout)
{
    __zn_udp_addr_t *laddr = (__zn_udp_addr_t *)laddr_arg;
...
{
    __zn_udp_addr_t *raddr = (__zn_udp_addr_t *)raddr_arg;
    __zn_udp_addr_t *laddr = NULL; // Multicast messages are not self-consumed,
...
void _zn_close_udp_multicast(void *sockrecv_arg, void *socksend_arg, void *arg)
{
...
size_t _zn_read_udp_multicast(void *sock_arg, uint8_t *ptr, size_t len, void *laddr_arg, z_bytes_t *addr)
{
    ...
    if (addr != NULL)
    {
        ...
        *addr = _z_bytes_make(strlen((const char*)&rip[0]) + strlen((const char*)&rip[1]) + strlen((const char*)&rip[2]) + strlen((const char*)&rip[3]) + sizeof(uint16_t));
        ...
        for (int i = 0; i < 4; i++)
        {
            memcpy((uint8_t *)addr->val + offset, &rip[i], strlen((const char*)&rip[i]));
            offset += strlen((const char*)&rip[i]);
        }
        memcpy((uint8_t *)addr->val + offset, &rport, sizeof(uint16_t));
    }
...
size_t _zn_read_exact_udp_multicast(void *sock_arg, uint8_t *ptr, size_t len, void *laddr_arg, z_bytes_t *addr)
{
    ...
    do
    {
        rb = _zn_read_udp_multicast(sock_arg, ptr, n, laddr_arg, addr);
        if (rb == SIZE_MAX)
            return rb;
