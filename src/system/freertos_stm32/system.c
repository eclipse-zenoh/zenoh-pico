...
/*------------------ Memory ------------------*/
void *z_malloc(size_t size) { return size > 0 ? pvPortMalloc(size) : BADPTR; }
...
void z_free(void *ptr) { if (ptr != BADPTR) vPortFree(ptr); }
