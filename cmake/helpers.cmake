...
function(zp_make_contract_target_name prefix name out_var)
  set(${out_var} "zenohpico::${prefix}_${name}" PARENT_SCOPE)
endfunction()

function(zp_ensure_interface_contract_target desired_target backing_target out_var)
  if(TARGET "${desired_target}")
    set(${out_var} "${desired_target}" PARENT_SCOPE)
    return()
  endif()

  if("${desired_target}" MATCHES "::")
    if(NOT TARGET "${backing_target}")
      add_library("${backing_target}" INTERFACE)
    endif()
    add_library("${desired_target}" ALIAS "${backing_target}")
    set(${out_var} "${backing_target}" PARENT_SCOPE)
  else()
    if(NOT TARGET "${desired_target}")
      add_library("${desired_target}" INTERFACE)
    endif()
    set(${out_var} "${desired_target}" PARENT_SCOPE)
  endif()
endfunction()

function(zp_collect_component_sources out_var)
  set(_zp_sources "")
  foreach(_zp_target IN LISTS ARGN)
    if("${_zp_target}" STREQUAL "" OR NOT TARGET "${_zp_target}")
      continue()
    endif()
    get_target_property(_zp_target_sources "${_zp_target}" INTERFACE_SOURCES)
    if("${_zp_target_sources}" MATCHES "-NOTFOUND$")
      set(_zp_target_sources "")
    endif()
    if(NOT "${_zp_target_sources}" STREQUAL "")
      list(APPEND _zp_sources ${_zp_target_sources})
    endif()
  endforeach()
  set(${out_var} "${_zp_sources}" PARENT_SCOPE)
endfunction()
